"""
Ingestion script for DDR dataset.

Dataset: DDR - A Large-scale Fundus Image Dataset for Diabetic Retinopathy Detection
Structure: Multi-task dataset with three annotation types:
  - DR_grading/: train/valid/test splits with .txt files (format: filename.jpg grade)
  - lesion_detection/: train/valid/test splits with XML files (Pascal VOC format)
  - lesion_segmentation/: train/valid/test splits with image/ and label/ folders
    - label/ (train/test) or "segmentation label/" (valid) contains: EX, HE, MA, SE
Annotations:
  - DR grading (6-level: 0=No DR, 1=Mild, 2=Moderate, 3=Severe, 4=PDR, 5=Ungradable)
  - Lesion detection (Pascal VOC bounding boxes: ex, ma, he, se)
  - Lesion segmentation (binary masks: EX, HE, MA, SE)
Tasks: DR grading, Localization (lesion bounding boxes), Segmentation (4 lesion types)
"""
...
def _get_label_dir(seg_split_dir: Path) -> Path:
    """Return the label directory for a segmentation split.

    The valid split uses 'segmentation label' (with a space) instead of 'label'.
    """
    label = seg_split_dir / "label"
    if label.exists():
        return label
    return seg_split_dir / "segmentation label"


def _count_grading_lines(file_path: Path) -> int:
    """Count non-empty lines in a grading text file."""
...
async def ingest_ddr() -> OperationStatistics:
    """Main ingestion function for DDR dataset."""
    data_root = get_data_root() / "43_DDR-dataset"
    ...
    await upsert_dataset(
        Dataset(
            ...
            task_types=["grading", "localization", "segmentation"],
            description=(
                "DDR is a large-scale fundus image dataset for diabetic retinopathy research "
                "with 12,522 images for DR grading (6-level scale including ungradable), "
                "757 images with lesion bounding boxes (Pascal VOC), and 757 images with "
                "binary lesion segmentation masks for 4 lesion types (EX, HE, MA, SE)."
            ),
        )
    )
    ...
    for split in SPLITS:
        ...
        if img_dir.exists():
            img_files = await asyncio.to_thread(find_images, img_dir, recursive=False)
    ...
    logger.info(
        ...
        f"(grading: {grading_count}, localization: {loc_count}, segmentation: {seg_count})"
    )

    tracker = ProgressTracker(total=total_count, description=f"Ingesting {DATASET_NAME}")

    # Shared image registry — filename is the stable key across all three tasks
    ...
    async def get_or_create_image(filename: str, image_path: Path, split_name: str) -> UUID:
        """Return existing image UUID or create a new Image record."""
        if filename in existing_image_index:
            img_id = existing_image_index[filename]
        ...
        existing_image_index[filename] = img_id
        image_to_splits.setdefault(img_id, set()).add(split_name)
        return img_id
    ...
    async def process_grading_line(line: str, _: int):
        try:
            ...
            image_path = data_root / "DR_grading" / split_name / filename
            ...
            grading = await process_disease_grade(
                ...
                image_id=image_id,
                scale_description="ICDR 6-level DR grading (0=No DR, 1=Mild, 2=Moderate, 3=Severe, 4=PDR, 5=Ungradable)",
                min_value=0,
                max_value=5,
    ...
    # ================================================================
    # LOCALIZATION (LESION DETECTION)
    # ================================================================
    logger.info("Processing lesion detection XML files...")
        ...
        try:
            ...
            if fn_elem is None or fn_elem.text is None:
                ...
                tracker.record_error(
                    ...
                    error_message="Missing filename in XML",
                    item_path=str(xml_path),
                )
                return
            filename = fn_elem.text.strip()

            # Image may live in grading dir or segmentation dir
    ...
    async def process_image_file(img_path: Path, *_):
        try:
            ...
            label_dir = _get_label_dir(img_path.parent.parent)
    ...
    # ================================================================
    # UPSERT (FK order: images → annotations)
    # ================================================================
    ...
    logger.info(
        f"  Total count (expected): {total_count} "
        f"(grading: {grading_count}, localization: {loc_count}, segmentation: {seg_count})"
    )
