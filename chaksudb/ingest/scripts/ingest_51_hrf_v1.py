"""
Ingestion script for HRF-v1 dataset.

Dataset: HRF-v1 — High-Resolution Fundus (version 1) with AV segmentation.
Structure: train/test split; disease type encoded in filename stem.
    training/images/*.png  — 30 fundus photographs
    training/av/*.png      — 28 color-coded AV masks (2 images lack masks)
...
# Suffix → list of (class_name, value) annotations
_DISEASE_SUFFIX: Dict[str, List] = {
    "dr": [("DR", True)],
    "g":  [("glaucoma", True)],
    "h":  [],  # healthy — no disease annotation needed
}


def _disease_suffix(stem: str) -> str:
    """Extract disease suffix from stem like '01_dr' → 'dr'."""
...
async def ingest_hrf_v1() -> OperationStatistics:
    data_root = get_data_root() / "51_HRF-v1"
    ...
    dataset = Dataset(
        ...
        description=(
            "HRF-v1 (High-Resolution Fundus version 1) provides 45 fundus images "
            "(30 training, 15 test) with color-coded artery/vein segmentation masks. "
